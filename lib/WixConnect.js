...
function fromBase64Safe(base64String) {
...
WixParameters.prototype = {
    ...
    toQueryString : function() {
        return _.reduce(this.params, function(queryOut, element) {
            return queryOut + ((queryOut.length > 0) ? '&' : '') + element.param + '=' + encodeURIComponent(element.value);
        }, '');
    },
...
module.exports = {
    ...
    parseInstance : function(instance, secret, validator) {
        ...
        var jsonData = JSON.parse(new Buffer(fromBase64Safe(data), 'base64').toString('utf8'));
