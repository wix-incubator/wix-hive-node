...
function Contacts(wixApi) {
     ...
     * @param {String} sessionToken The contact session token to match with, provided by the JS SDK
     * @returns {Promise.<Object, error>} A Contact id, or an error
     */
    this.upsert = function(phone, email, sessionToken) {
        ...
        if (!upsertData.phone && !upsertData.email && !upsertData.sessionToken) {
...
function Services(wixApi) {
     ...
     * @returns {*}
     */
    this.getProviders = function(serviceType) {
        if(!(serviceType instanceof ServiceType)) {
            throw 'WixServiceType must be provided'
        }
        var api = this;

        return this.resourceRequest(api.createRequest("GET", "/v1/services/" + serviceType.name + "/providers"),null);
    };
}
Services.prototype = new WixAPICaller();
...
function Wix(secretKey, appId, instanceId) {
    ...
    /**
     * An interface to interact with Wix Services
     * @public
     * @member
     * @readonly
     * @type Services
     * */
    this.Services = new Services(this).withSecretKey(secretKey).withAppId(appId).withInstanceId(instanceId);
    /**
     * An interface to interact with Wix Sites
     * @public
     * @member
