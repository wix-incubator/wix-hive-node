...
function BaseWixAPIObject() {
    ...
    this.TYPES = {
         ...
         * @constant
         */
        ECOMM_PURCHASE : new ActivityType("e_commerce/purchase"),
        /**
...
function WixActivity() {
     ...
     * @type {Object}
     */
    this.contactUpdate = schemaFactory(this.TYPES.CONTACT_CREATE.schema);
    /**
...
function Activities(wixApi) {
    ...
    this.postActivity = function(activity, userSessionToken) {
        ...
        request.asWixQueryParams();
        var options = request.toHttpsOptions();
        rest.postJson('http://' + options.host + options.path, activity, {
            headers : options.headers
        }).on('complete', function(data, response) {
            if(response.statusCode === 200) {
...
function Contacts(wixApi) {
    ...
    this.create = function(contact) {
        ...
        var options = request.toHttpsOptions();
        rest.postJson('http://' + options.host + options.path, contact.toJson(), {
            headers : options.headers
        }).on('complete', function(data, response) {
            if(response.statusCode === 200) {
    ...
    this.upsert = function(phone, email) {
        ...
        request.asWixHeaders();
        var options = request.toHttpsOptions();
        rest.putJson('http://' + options.host + options.path, upsertData, {
            headers : options.headers
        }).on('complete', function(data, response) {
            if(response.statusCode === 200) {
...
Contact.prototype.saveEditedField = function(path, obj, isListField){
    ...
    request.asWixHeaders();
    var options = request.toHttpsOptions();
    rest.putJson('http://' + options.host + options.path, obj.toJson(), {
        headers : options.headers
    }).on('complete', function(data, response) {
        if(response.statusCode === 200) {
...
Contact.prototype.saveNewField = function(path, obj) {
    ...
    request.asWixHeaders();
    var options = request.toHttpsOptions();
    rest.postJson('http://' + options.host + options.path, obj.toJson(), {
        headers : options.headers
    }).on('complete', function(data, response) {
        if(response.statusCode === 200) {
...
Contact.prototype.update = function() {
    ...
    var request = wixApi.createRequest("POST", "/v1/contacts/" + contact.id().id());
    var deferred = q.defer();
    request.withPostData(JSON.stringify(contact.toJson()));
    request.asWixHeaders();
    var options = request.toHttpsOptions();
